...
def _is_zero_reserve_error(
    exc: Exception,
) -> bool:
    """Return True when Pump.fun's bonding curve has no usable reserves."""
    text = str(exc).lower()
    normalized = (
        text
        .replace(" ", "")
    ...
    markers = (
        "virtualtokenreservesarezero",
        "virtualsolreservesarezero",
        "virtualtokensupplyiszero",
        "calculatedtokenpriceisnonpositive",
    )
    return any(marker in normalized for marker in markers)


def _is_bonding_curve_graduated_error(
    exc: Exception,
) -> bool:
    """True when a bonding-curve SELL was rejected on-chain because the
    curve's own reserve account is uninitialized (Anchor error 3012 /
    0xbc4 on associatedbondingcurve specifically).

    A pre-flight migration check (see _is_migrated) can still lose a
    race: the token can graduate in the gap between that check and this
    transaction actually reaching a validator for simulation, since
    migration withdraws the bonding curve's reserves out from under it.
    This is the on-chain program itself confirming that happened, which
    is more authoritative than any pre-check - so it's treated as a
    signal to retry immediately through Jupiter rather than a plain
    failure.
    """
    ...
    return (
        "associatedbondingcurve" in text
        and (
            "accountnotinitialized" in text
            or "0xbc4" in text
            or "3012" in text
        )
    )
    ...
    async def sell(
        ...
        for attempt in range(
            ...
            try:
                ...
                #
                # force_jupiter short-circuits this check once that
                # exact on-chain error has already been seen this call
                # (see the except block below) - at that point we know
                # for certain the curve is gone, no need to ask again.
                # ------------------------------------------------------

                is_migrated = (
                    force_jupiter
                    or await self._is_migrated(
                        token
                    )
                )
                ...
                if (
                    ...
                    and (
                        _is_bonding_curve_graduated_error(exc)
                        or _is_zero_reserve_error(exc)
                    )
                ):
                    ...
                    logger.warning(
                        "pumpfun_sell_graduated_mid_attempt_retrying_via_jupiter",
                        extra={
                            "mint": token.mint,
                            "attempt": (
                                attempt + 1
