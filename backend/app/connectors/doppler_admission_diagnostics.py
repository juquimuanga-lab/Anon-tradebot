...
def install() -> None:
    ...
    async def traced_normalize(item, doppler, doppler_control):
        ...
        if accepted:
            logger.info(
                "doppler_admission_accepted",
                extra={"mint": mint, "state_numeraire": item.get("numeraire"), "currency0": pool_key.get("currency0"), "currency1": pool_key.get("currency1"), "status": state.get("status"), "tokens_on_curve": state.get("tokens_on_curve")},
            )
        else:
            logger.warning(
                "doppler_admission_rejected",
                extra={"mint": mint, "event_numeraire": event_numeraire, "state_numeraire": item.get("numeraire"), "currency0": pool_key.get("currency0"), "currency1": pool_key.get("currency1"), "status": state.get("status"), "tokens_on_curve": state.get("tokens_on_curve"), "reason": "initializer state did not prove canonical SPCX/currency0 -> launched token/currency1"},
            )
        return accepted
