        ...
        if owner_user_id is not None:
            ...
            if legacy:
                state = BotState(
                    ...
                    guardian_enabled=getattr(legacy, "guardian_enabled", True),
                    guardian_auto_pause_enabled=getattr(legacy, "guardian_auto_pause_enabled", True),
                    guardian_last_status=getattr(legacy, "guardian_last_status", "HEALTHY"),
                    guardian_pause_reason=getattr(legacy, "guardian_pause_reason", None),
        ...
        state = (
            await session.execute(
                select(BotState).where(
                    BotState.owner_user_id == admin_id
                )
            )
        ).scalar_one_or_none()
...
async def get_open_positions(owner_user_id: Optional[int] = None) -> list[Position]:
...
async def get_recent_orders(
    ...
    async with async_session_scope() as session:
        query = select(Order)
        if owner_user_id is not None:
            query = query.where(Order.owner_user_id == owner_user_id)
