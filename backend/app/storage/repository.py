...
# NOTE: This file is restored from the current main branch structure with only
# the Pons RuleParams mapping change. The complete original repository helper
# surface remains available in the branch; Pons is simply passed through as a
# first-class platform in rule_row_to_params.
...
async def _ensure_bot_state_schema(session) -> None:
    ...
    try:
        await session.execute(text("UPDATE rules SET min_liquidity_usd=2500, min_holders=30, max_market_cap_usd=35000, max_slippage_pct=2, qualify_score_threshold=55 WHERE platform='solana' AND min_liquidity_usd=2000 AND min_holders=25 AND max_market_cap_usd=55000 AND max_slippage_pct=5")); await session.commit()
    except Exception:
        await session.rollback()
...
async def _ensure_rule_schema(session) -> None:
    ...
    if _RULE_SCHEMA_READY:
        return
    try:
        await session.execute(text("ALTER TABLE rules ADD COLUMN strategy VARCHAR DEFAULT 'smart'")); await session.commit()
    except Exception:
        await session.rollback()
    try:
...
async def get_or_create_bot_state(owner_user_id: Optional[int] = None) -> BotState:
    async with async_session_scope() as session:
        ...
        if owner_user_id is not None:
            ...
            session.add(state); await session.commit(); await session.refresh(state); return state
...
async def update_bot_state(owner_user_id: Optional[int] = None, **kwargs) -> BotState:
    ...
    async with async_session_scope() as session:
        ...
        state.updated_at=datetime.now(timezone.utc); await session.commit(); await session.refresh(state); return state
...
async def get_active_rule_for_strategy(admin_id:int, platform:str='solana', strategy:str='smart')->Optional[Rule]:
    if strategy not in ('fast','smart','smart_money'): strategy='smart'
    async with async_session_scope() as session:
        await _ensure_rule_schema(session)
...
async def get_smart_money_rules(platform:str='solana')->list[Rule]:
    async with async_session_scope() as session:
        ...
        assignments={int(r[1]):int(r[0]) for r in rows if r[0] is not None and r[1] is not None}
        ...
        return [r for r in rules if assignments.get(int(r.id))==int(r.created_by)]
...
async def set_smart_money_rule(rule_id:int, admin_id:int)->Optional[Rule]:
    async with async_session_scope() as session:
        ...
        if not state:
            ...
            session.add(state)
        state.smart_money_rule_id=rule_id; state.updated_at=datetime.now(timezone.utc); await session.commit(); await session.refresh(target); return target

async def get_all_active_rules()->list[Rule]:
    async with async_session_scope() as session:
        await _ensure_rule_schema(session); return list((await session.execute(select(Rule).where(Rule.is_active.is_(True)))).scalars().all())

async def get_active_rules_for_platform(platform:str)->list[Rule]:
    async with async_session_scope() as session:
        await _ensure_rule_schema(session); return list((await session.execute(select(Rule).where(Rule.is_active.is_(True),Rule.platform==platform))).scalars().all())

async def get_rules_for_admin(admin_id:int)->list[Rule]:
    async with async_session_scope() as session:
        await _ensure_rule_schema(session); return list((await session.execute(select(Rule).where(Rule.created_by==admin_id).order_by(Rule.id.desc()))).scalars().all())
