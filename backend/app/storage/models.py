...
class Token(Base):
    ...
    ticker_name: Mapped[str] = mapped_column(String, default="")
    ticker_symbol: Mapped[str] = mapped_column(String, default="")
    creator_wallet: Mapped[str] = mapped_column(String, default="", index=True)
    created_on: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    source: Mapped[str] = mapped_column(String, default="anoncoin")
...
class ScreeningResult(Base):
    ...
    creator_match: Mapped[bool] = mapped_column(Boolean, default=False)
...
class Rule(Base):
    ...
    max_buy_size_sol: Mapped[float] = mapped_column(Float, default=0.1)
    min_liquidity_usd: Mapped[float] = mapped_column(Float, default=1000.0)
    min_holders: Mapped[int] = mapped_column(Integer, default=10)
    max_age_seconds: Mapped[int] = mapped_column(Integer, default=600)
    creator_allowlist: Mapped[list] = mapped_column(JSON, default=list)
    creator_denylist: Mapped[list] = mapped_column(JSON, default=list)
    bonding_curve_phase: Mapped[str] = mapped_column(String, default="any")
    min_market_cap_usd: Mapped[float | None] = mapped_column(Float, nullable=True)
    max_market_cap_usd: Mapped[float | None] = mapped_column(Float, nullable=True)
    max_slippage_pct: Mapped[float] = mapped_column(Float, default=5.0)
    max_trades_per_hour: Mapped[int] = mapped_column(Integer, default=5)
    cooldown_seconds: Mapped[int] = mapped_column(Integer, default=120)
    take_profit_levels: Mapped[list] = mapped_column(JSON, default=list)
    ...
    sell_on_volume_drop_pct: Mapped[float | None] = mapped_column(Float, nullable=True)
    time_based_exit_seconds: Mapped[int | None] = mapped_column(Integer, nullable=True)
...
class TradeDecision(Base):
    ...
    decision: Mapped[str] = mapped_column(String, default="skip")
...
class Order(Base):
    ...
    requested_amount_sol: Mapped[float] = mapped_column(Float, default=0.0)
...
class Position(Base):
    ...
    amount_tokens: Mapped[float] = mapped_column(Float, default=0.0)
...
class BotState(Base):
    ...
    paper_balance_sol: Mapped[float] = mapped_column(Float, default=10.0)
