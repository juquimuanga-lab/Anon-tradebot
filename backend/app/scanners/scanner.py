...
# unchanged.
FAST_EARLY_ENTRY_MAX_AGE_SECONDS = 8.0
# Keep Fast candidates alive long enough for the global launch-safety RPC
# check to recover from a transient zero-transaction response. The early
# entry gate remains intentionally tighter; after it expires, the normal Fast
# filters can still qualify the token while it remains within the rule age.
FAST_CANDIDATE_RETRY_WINDOW_SECONDS = 20.0
                        ...
                        if (
                            ...
                            effective_age = (
                                max(
                                    5.0,
                                    min(
                                        float(rule.max_age_seconds),
                                        FAST_CANDIDATE_RETRY_WINDOW_SECONDS,
                                    ),
                                )
                                if strategy == "fast"
