...
GRADUATION_HUNTER_MIN_REAL_SOL = 3.0
...
class ScannerService:
    ...
    async def _build_pons_snapshot(self, mint, metadata, first_seen):
        ...
        if float(market.get("price_usd", 0.0) or 0.0) <= 0:
            logger.warning("pons_snapshot_not_ready", extra={"mint": mint, "error": "ETH/USD price unavailable or invalid Pons price"})
            return None

        # The factory graduation threshold is denominated in the native ETH
        # quote asset (18 decimals). Keep both the raw value and the normalized
        # ETH value so the hunter never confuses ERC-20 token decimals with the
        # quote reserve decimals.
        threshold_raw = int(market.get("graduation_threshold", 0) or 0)
        threshold_eth = threshold_raw / 1_000_000_000_000_000_000
        progress_pct = float(market.get("progress_pct", 0.0) or 0.0)
        real_quote_eth = threshold_eth * progress_pct / 100.0 if threshold_eth > 0 else 0.0
        ...
        token = TokenSnapshot(
            ...
            raw_enrichment={
                ...
                "graduation_network": "robinhood",
                "graduation_quote_asset": "ETH",
                "graduation_quote_reserve_raw": threshold_raw * progress_pct / 100.0,
                "graduation_quote_reserve_eth": real_quote_eth,
                "graduation_threshold_eth": threshold_eth,
                "graduation_progress_pct": progress_pct,
                "token_decimals": int(market.get("decimals", 18)),
            },
        )
        # Dynamic attributes keep compatibility with the existing TokenSnapshot
        # dataclass while exposing network-native graduation telemetry.
        token.graduation_quote_reserve_native = real_quote_eth
        token.graduation_quote_symbol = "ETH"
        token.graduation_quote_decimals = 18
        token.graduation_quote_progress_pct = progress_pct
        return token
    ...
    def _update_graduation_history(self, token) -> dict:
        """Track network-native curve reserve and holder acceleration."""
        now = time.monotonic()
        state = self._graduation_history.get(token.mint) or {}
        previous = {
            "timestamp": state.get("timestamp", 0.0),
            "holders": state.get("holders", 0),
            "reserve": state.get("reserve", 0.0),
            "reserve_unit": state.get("reserve_unit", "SOL" if token.source == SOURCE_PUMPFUN else "ETH"),
        }
        if token.source == SOURCE_PUMPFUN:
            current_reserve = float(getattr(token, "real_sol_reserves_sol", 0.0) or 0.0)
            reserve_unit = "SOL"
        elif token.source == SOURCE_PONS:
            current_reserve = float(getattr(token, "graduation_quote_reserve_native", 0.0) or 0.0)
            reserve_unit = "ETH"
        else:
            current_reserve = 0.0
            reserve_unit = "native"
        ...
        if dt > 0.25 and previous.get("holders") is not None:
            holder_growth_per_min = max(
                0.0,
                (current_holders - int(previous.get("holders", 0) or 0)) / dt * 60.0,
            )
        ...
        # Preserve the old key for Pump.fun compatibility.
        if token.source == SOURCE_PUMPFUN:
            history["real_sol_reserves"] = current_reserve
        else:
            history["real_quote_reserve_eth"] = current_reserve
    ...
    def _graduation_hunter_gate(token, rule_params) -> tuple[str, float, list[str], dict]:
        """Evaluate graduation progress using network-native quote reserves.

        Pump.fun is measured in real SOL reserves. Pons/Robinhood is measured
        in real ETH quote reserves. Token amounts are never compared in raw
        units across networks; the Pons snapshot already normalizes its ERC-20
        token decimals before producing price/market-cap values.
        """
        if not getattr(rule_params, "graduation_hunter_enabled", True):
        ...
        configured_max_age = float(getattr(rule_params, "graduation_hunter_max_observation_seconds", GRADUATION_HUNTER_MAX_OBSERVATION_SECONDS) or GRADUATION_HUNTER_MAX_OBSERVATION_SECONDS)
        max_age = max(configured_max_age, GRADUATION_HUNTER_MAX_OBSERVATION_SECONDS)

        network = "solana" if token.source == SOURCE_PUMPFUN else "robinhood" if token.source == SOURCE_PONS else str(token.source)

        if age < min_age:
            return "defer", 0.0, [f"graduation hunter observing ({age:.1f}s/{min_age:.0f}s minimum)"], {
                "observation_ready": False, "network": network,
            }
        if age > max_age:
            return "reject", 0.0, [f"graduation hunter window expired ({age:.1f}s > {max_age:.0f}s)"], {
                "observation_ready": True, "network": network,
            }

        # --------------------------------------------------------------
        # Robinhood/Pons: use the curve's native ETH quote reserve.
        # --------------------------------------------------------------
        if token.source == SOURCE_PONS:
            market = (getattr(token, "raw_enrichment", {}) or {}).get("pons") or {}
            progress = float(market.get("progress_pct", 0.0) or 0.0)
            threshold_raw = int(market.get("graduation_threshold", 0) or 0)
            threshold_eth = threshold_raw / 1_000_000_000_000_000_000
            quote_eth = threshold_eth * progress / 100.0 if threshold_eth > 0 else 0.0
            ready = bool(market.get("ready_to_graduate", False))
            graduated = bool(market.get("graduated", False) or market.get("is_migrated", False))
            token_decimals = int(market.get("decimals", getattr(token, "decimals", 18)) or 18)
            eth_usd = float(market.get("eth_usd", 0.0) or 0.0)
            price_eth = float(market.get("price_eth", 0.0) or 0.0)

            # Pons hunter uses the graduation threshold itself as the scale,
            # so it remains correct even when individual tokens have different
            # ERC-20 decimals or different configured graduation thresholds.
            min_progress = 10.0
            max_progress = 75.0
            if graduated or ready or progress >= 100.0:
                return "reject", 0.0, [
                    f"Pons curve already at graduation ({progress:.1f}% of threshold)"
                ], {
                    "observation_ready": True,
                    "network": "robinhood",
                    "quote_asset": "ETH",
                    "quote_reserve_eth": round(quote_eth, 8),
                    "graduation_threshold_eth": round(threshold_eth, 8),
            ...
            if progress < min_progress:
                return "defer", 0.0, [
                    f"real curve reserves {quote_eth:.4f} ETH below {min_progress:.0f}% of graduation threshold"
                ], {
                    "observation_ready": True,
                    "network": "robinhood",
                    "quote_asset": "ETH",
            ...
            holder_history = (getattr(token, "raw_enrichment", {}) or {}).get("graduation_history") or {}
            holder_growth = float(holder_history.get("holder_growth_per_minute", 0.0) or 0.0)

            # Network-independent 100-point Pons Hunter score. Reserve progress
            # is the primary signal; holders/liquidity/MC provide confirmation.
            progress_score = 40.0 if min_progress <= progress <= max_progress else 25.0
            progress_score += min(15.0, max(0.0, (progress - min_progress) / max(max_progress - min_progress, 1.0) * 15.0))
            holder_score = min(15.0, holders / 20.0 * 15.0)
            liquidity_score = min(15.0, max(0.0, float(token.liquidity_usd or 0.0) / 1500.0 * 15.0))
            age_score = 10.0 if age <= 90.0 else 7.0 if age <= 180.0 else 3.0
            growth_score = min(5.0, max(0.0, holder_growth / 15.0 * 5.0))
            score = round(min(100.0, progress_score + holder_score + liquidity_score + age_score + growth_score), 2)
            threshold_score = float(getattr(rule_params, "graduation_hunter_score_threshold", GRADUATION_HUNTER_SCORE_THRESHOLD) or GRADUATION_HUNTER_SCORE_THRESHOLD)
            ...
            breakdown = {
                ...
                "liquidity_usd": round(float(token.liquidity_usd or 0.0), 2),
                "market_cap_usd": round(float(token.market_cap_usd or 0.0), 2),
                "score_threshold": threshold_score,
            }
            if score < threshold_score:
                return "defer", score, [f"graduation score {score:.1f} below {threshold_score:.1f}"], breakdown
            return "pass", score, [], breakdown

        # --------------------------------------------------------------
        # Solana/Pump.fun: preserve the existing SOL-native logic.
        # --------------------------------------------------------------
        ...
        if token.source != SOURCE_PUMPFUN:
            return "pass", 100.0, [], {"enabled": False, "network": network}
        if reserves < min_sol:
            return "defer", 0.0, [f"real curve reserves {reserves:.2f} SOL below {min_sol:.2f} SOL entry floor"], {"observation_ready": True, "network": "solana", "real_sol_reserves": round(reserves, 4)}
        if reserves > max_sol:
            return "reject", 0.0, [f"real curve reserves {reserves:.2f} SOL already above {max_sol:.2f} SOL entry ceiling"], {"observation_ready": True, "network": "solana", "real_sol_reserves": round(reserves, 4)}
        ...
        breakdown.update({"observation_ready": True, "network": "solana", "age_seconds": round(age, 2)})
        if reasons:
    ...
    async def _screen_and_maybe_trade(
        ...
        # ------------------------------------------------------------------
        # GRADUATION HUNTER (Solana/Pump.fun + Robinhood/Pons Smart lanes)
        # ------------------------------------------------------------------
        if (
            token.source in {SOURCE_PUMPFUN, SOURCE_PONS}
            and strategy == "smart"
            and GRADUATION_HUNTER_ENABLED
            and getattr(rule_params, "graduation_hunter_enabled", True)
        ):
            ...
            hunter_log = {
                ...
                "rule_id": rule.id,
                "source": token.source,
                "network": hunter_breakdown.get("network"),
                "status": hunter_status,
                "score": hunter_score,
                "reasons": hunter_reasons,
                "breakdown": hunter_breakdown,
            ...
            if token.source == SOURCE_PONS:
                hunter_log.update({
                    "quote_asset": "ETH",
                    "quote_reserve_eth": hunter_breakdown.get("quote_reserve_eth"),
                    "graduation_threshold_eth": hunter_breakdown.get("graduation_threshold_eth"),
                    "progress_pct": hunter_breakdown.get("progress_pct"),
                    "token_decimals": hunter_breakdown.get("token_decimals"),
                })
            else:
                hunter_log.update({
                    "quote_asset": "SOL",
                    "real_sol_reserves": hunter_breakdown.get("real_sol_reserves", getattr(token, "real_sol_reserves_sol", 0.0)),
                })
            logger.info("graduation_hunter_evaluation " + json.dumps(hunter_log, default=str, separators=(",", ":"), sort_keys=True))
    ...
    async def _process_watched_wallet_pending(
        ...
        for mint in list(
            ...
            if smart_rules:
                ...
                if source in {SOURCE_PUMPFUN, SOURCE_PONS}:
