...
GRADUATION_HUNTER_MAX_OBSERVATION_SECONDS = 600.0
GRADUATION_HUNTER_MIN_REAL_SOL = 3.0
GRADUATION_HUNTER_MAX_REAL_SOL = 35.0
GRADUATION_HUNTER_TARGET_REAL_SOL = 85.0
GRADUATION_HUNTER_MIN_BUY_SELL_RATIO = 1.5
GRADUATION_HUNTER_MIN_UNIQUE_BUYERS = 5
GRADUATION_HUNTER_MIN_BUYER_DIVERSITY = 0.20
...
# Strong organic-flow override: allow a high-momentum launch through the
# score gate when the independent safety gate is clean. Hard safety rejects
# always win.
GRADUATION_HUNTER_MOMENTUM_OVERRIDE_ENABLED = True
GRADUATION_HUNTER_MOMENTUM_MIN_BUY_PRESSURE = 0.70
GRADUATION_HUNTER_MOMENTUM_MIN_BUY_SELL_RATIO = 2.50
GRADUATION_HUNTER_MOMENTUM_MIN_BUY_VELOCITY_SOL_PER_SEC = 0.40
GRADUATION_HUNTER_MOMENTUM_MIN_UNIQUE_BUYERS = 5
GRADUATION_HUNTER_MOMENTUM_MAX_TOP_BUYER_SHARE = 0.50
GRADUATION_HUNTER_MOMENTUM_MAX_TOP3_BUYER_SHARE = 0.85
GRADUATION_HUNTER_MOMENTUM_MIN_BUYER_DIVERSITY = 0.20
...
class ScannerService:
    ...
    def _graduation_hunter_gate(token, rule_params) -> tuple[str, float, list[str], dict]:
        ...
        buy_velocity = float(sig.get("buy_velocity_sol_per_sec", 0.0) or 0.0)
        top_buyer_share = float(sig.get("top_buyer_share", 1.0) or 1.0)
        top3_buyer_share = float(sig.get("top3_buyer_share", 1.0) or 1.0)
        ...
        momentum_override = (
            GRADUATION_HUNTER_MOMENTUM_OVERRIDE_ENABLED
            and bool(getattr(rule_params, "graduation_hunter_momentum_override_enabled", True))
            and buy_pressure >= float(getattr(rule_params, "graduation_hunter_momentum_min_buy_pressure", GRADUATION_HUNTER_MOMENTUM_MIN_BUY_PRESSURE) or GRADUATION_HUNTER_MOMENTUM_MIN_BUY_PRESSURE)
            and buy_sell_ratio >= float(getattr(rule_params, "graduation_hunter_momentum_min_buy_sell_ratio", GRADUATION_HUNTER_MOMENTUM_MIN_BUY_SELL_RATIO) or GRADUATION_HUNTER_MOMENTUM_MIN_BUY_SELL_RATIO)
            and buy_velocity >= float(getattr(rule_params, "graduation_hunter_momentum_min_buy_velocity_sol_per_sec", GRADUATION_HUNTER_MOMENTUM_MIN_BUY_VELOCITY_SOL_PER_SEC) or GRADUATION_HUNTER_MOMENTUM_MIN_BUY_VELOCITY_SOL_PER_SEC)
            and unique_buyers >= int(getattr(rule_params, "graduation_hunter_momentum_min_unique_buyers", GRADUATION_HUNTER_MOMENTUM_MIN_UNIQUE_BUYERS) or GRADUATION_HUNTER_MOMENTUM_MIN_UNIQUE_BUYERS)
            and top_buyer_share <= float(getattr(rule_params, "graduation_hunter_momentum_max_top_buyer_share", GRADUATION_HUNTER_MOMENTUM_MAX_TOP_BUYER_SHARE) or GRADUATION_HUNTER_MOMENTUM_MAX_TOP_BUYER_SHARE)
            and top3_buyer_share <= float(getattr(rule_params, "graduation_hunter_momentum_max_top3_buyer_share", GRADUATION_HUNTER_MOMENTUM_MAX_TOP3_BUYER_SHARE) or GRADUATION_HUNTER_MOMENTUM_MAX_TOP3_BUYER_SHARE)
            and buyer_diversity >= float(getattr(rule_params, "graduation_hunter_momentum_min_buyer_diversity", GRADUATION_HUNTER_MOMENTUM_MIN_BUYER_DIVERSITY) or GRADUATION_HUNTER_MOMENTUM_MIN_BUYER_DIVERSITY)
        )

        if result.score < threshold and momentum_override:
            breakdown["momentum_override"] = True
            breakdown["momentum_override_buy_pressure"] = round(buy_pressure, 4)
            breakdown["momentum_override_buy_sell_ratio"] = round(buy_sell_ratio, 4)
            breakdown["momentum_override_buy_velocity_sol_per_sec"] = round(buy_velocity, 6)
            breakdown["momentum_override_unique_buyers"] = unique_buyers
            breakdown["momentum_override_top_buyer_share"] = round(top_buyer_share, 4)
            breakdown["momentum_override_top3_buyer_share"] = round(top3_buyer_share, 4)
            return "pass", result.score, [
                f"momentum override: {buy_pressure:.0%} buy pressure, {buy_sell_ratio:.2f} buy/sell, {buy_velocity:.3f} SOL/s, {unique_buyers} buyers"
            ], breakdown
