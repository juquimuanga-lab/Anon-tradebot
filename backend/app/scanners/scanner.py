...
class ScannerService:
    ...
    async def _screen_and_maybe_trade(
        ...
        # path. Smart Money returned through the blind-copy path above.
    ...
    async def scan_once(
        ...
        # Smart-money copy is an event-driven lane. Resolve the assigned
        # Smart Money rule directly so the copy signal cannot be lost merely
        # because the rule is not present/matched in the generic active-rule
        # iteration. Keep the generic rules for normal launch processing.
        smart_money_rules = []
        if settings.smart_money_enabled:
            ...
            for rule in active_rules:
                try:
                    ...
                    if assigned and assigned.id == rule.id and rule.id not in seen_rule_ids:
                        smart_money_rules.append(rule)
                        seen_rule_ids.add(rule.id)
                except Exception:
                    logger.exception(
                        "smart_money_rule_resolution_failed",
                        extra={"rule_id": getattr(rule, "id", None), "owner_id": getattr(rule, "created_by", None)},
                    )
    ...
    async def run_forever(
        ...
        while True:
            try:
                ...
                # Smart Money is an event-driven lane. Resolve the currently
                # assigned Smart Money rule explicitly here as well as in
                # scan_once(). This is critical because Railway normally uses
                # run_forever(), and previously this call omitted
                # smart_money_rules, causing queued Smart Money events to fall
                # back to the generic Fast/Smart rule list.
                smart_money_rules = []
                if settings.smart_money_enabled:
                    seen_rule_ids = set()
                    for rule in active_rules:
                        try:
                            ...
                            if (
                                assigned
                                and assigned.id == rule.id
                                and rule.id not in seen_rule_ids
                            ):
                                smart_money_rules.append(rule)
                                seen_rule_ids.add(rule.id)
                        except Exception:
                            logger.exception(
                                ...
                                extra={
                                    ...
                                    "owner_id": getattr(rule, "created_by", None),
                                },
                            )
