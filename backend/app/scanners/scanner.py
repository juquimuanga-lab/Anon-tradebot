...
2. Pump.fun
   Detected independently through the official Pump.fun program/create-instruction watcher.
...
class ScannerService:
    ...
    async def _watch_smart_money_buys(self):
        ...
        for event in events:
            ...
            # IMPORTANT: Smart Money detection must never be blocked by Guardian
            # telemetry. Guardian persistence/DB work is auxiliary; if it is
            # slow or raises, the trading event must still reach the scanner
            # processing path. The previous implementation awaited Guardian
            # before emitting the queue log/returning, which could stall the
            # entire scanner cycle and leave every later WSS BUY undrained.
            logger.info(
            ...
            # Guardian telemetry is best-effort and must not be allowed to
            # interrupt Smart Money queue handoff. Schedule it after the event
            # has been durably placed in the in-memory pending lane.
            async def _record_smart_money_telemetry(event=event, mint=mint):
                try:
                    await guardian.record(
                        "candidate", source=SOURCE_PUMPFUN,
                        smart_money=True, mint=mint,
                    )
                    await guardian.record(
                        "smart_money_buy",
                        wallet=event.get("wallet"),
                    ...
                    logger.debug(
                        "smart_money_guardian_telemetry_failed",
                        exc_info=True,
                        extra={"mint": mint},
                    )

            asyncio.create_task(
                _record_smart_money_telemetry(),
                name=f"smart-money-telemetry-{mint[:8]}",
            )
    ...
    async def _watch_pumpfun_for_new_mints(
        ...
        try:
            ...
            logger.exception(
                ...
                extra={
                    "program_id": onchain_watcher.PUMPFUN_PROGRAM_ID,
                },
            )
    ...
    async def _get_pumpfun_pool_info(
        ...
        for attempt, delay in enumerate(
            ...
            try:
                ...
                last_exc = exc
                message = str(exc)
                missing_account = self._is_missing_account_error(exc)
                if missing_account:
                    if attempt < len((0.0,) + PUMPFUN_RPC_RETRY_DELAYS):
                        logger.info(
                            "pumpfun_curve_not_visible_yet",
                            extra={"mint": mint, "attempt": attempt, "error": message},
                        )
                        continue
                if (
                    "429" not in message
    ...
    async def _process_watched_wallet_pending(
        ...
        if self._pending_watch:
            smart_pending = sum(
                1 for item in self._pending_watch.values()
                if item.get("smart_money")
            )
            logger.info(
                "watched_wallet_pending_cycle",
                extra={
                    "pending_total": len(self._pending_watch),
                    "smart_money_pending": smart_pending,
