...
class ScannerService:
    ...
    async def _maybe_trade(
        ...
        # ------------------------------------------------------------------
        # This is intentionally NOT a Telegram ruleset. It applies beneath
        # whichever admin ruleset/strategy is active and is designed to catch
        # obvious coordinated/bundled launches without changing RuleParams.
        #
        # Smart-money copy remains an independent signal path and keeps its
        # existing blind-copy behavior; normal Fast/Smart Pump.fun entries are
        # protected by this gate.
        if (
            token.source == SOURCE_PUMPFUN
            and not token.raw_enrichment.get("smart_money")
        ):
