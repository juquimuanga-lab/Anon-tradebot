    ...
    async def _process_watched_wallet_pending(
        ...
        for mint in list(
            ...
            if not due_rules:
                if is_smart_money:
                    eligibility_reasons = []
                    for rule in (smart_money_rules or []):
                        rule_reason = None
                        if not _rule_matches_source(rule, source):
                            rule_reason = "platform_mismatch"
                        elif source == SOURCE_PUMPFUN:
                            state = await repo.get_or_create_bot_state(rule.created_by)
                            if not state.trading_enabled:
                                rule_reason = "trading_disabled"
                            elif not state.pumpfun_trading_enabled:
                                rule_reason = "pumpfun_trading_disabled"
                            elif _rule_strategy(rule) != "smart_money":
                                rule_reason = "wrong_strategy"
                            elif not getattr(state, "smart_money_copy_enabled", False):
                                rule_reason = "smart_money_copy_disabled"
                        if rule_reason:
                            eligibility_reasons.append({"rule_id": rule.id, "owner_id": rule.created_by, "reason": rule_reason})
                    logger.warning(
