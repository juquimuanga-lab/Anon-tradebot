...
class ScannerService:
    ...
    async def _maybe_trade(
        ...
        # Pons has a separate persistent network mode. It never changes the
        # Solana /live or /paper state. trading_enabled remains the global kill switch.
        execution_mode = state.mode
        if token.source == SOURCE_PONS:
            execution_mode = (await secrets_manager.get_pons_mode(rule_row.created_by)) or "paper"
        ...
        if token.source == SOURCE_PONS:
            if not getattr(settings, "robinhood_pons_trading_enabled", False):
                await repo.save_trade_decision(
                    token.mint, rule_row.id, "skip",
                    "Pons/Robinhood trading is disabled at deployment level", score_result.score
                )
                return True
            if execution_mode == "live" and not await secrets_manager.get_robinhood_wallet_private_key(rule_row.created_by):
                await repo.save_trade_decision(
                    token.mint, rule_row.id, "skip",
                    "Pons live mode needs a connected Robinhood Chain wallet; use /connectrobinhoodwallet", score_result.score
                )
                return True
        ...
        if token.source == SOURCE_FOURMEME:
            amount_native = float(getattr(rule_row, "max_buy_size_bnb", 0.01) or 0.01)
            amount_unit = "BNB"
        elif token.source == SOURCE_PONS:
            # Legacy DB field max_buy_size_sol is reused as a numeric native amount
            # for Pons; the rule platform makes the unit explicitly ETH.
            amount_native = float(rule_row.max_buy_size_sol or 0.005)
            amount_unit = "ETH"
        else:
            amount_native = min(
                rule_row.max_buy_size_sol,
                (state.paper_balance_sol if execution_mode == "paper" else rule_row.max_buy_size_sol),
            )
            amount_unit = "SOL"
        ...
        if token.source == SOURCE_PONS and hasattr(self._notifier, "_send_to"):
            await self._notifier._send_to(
                rule_row.created_by,
                f"*Pons buy attempt* `{token.ticker_symbol or token.mint[:8]}` for {amount_native:.6f} ETH ({execution_mode} mode)\n"
        ...
        if result.success:
            ...
            if token.source == SOURCE_PONS and hasattr(self._notifier, "_send_to"):
                await self._notifier._send_to(
                    rule_row.created_by,
                    f"*Pons buy filled* `{token.ticker_symbol or token.mint[:8]}` @ ${fill_price:.6f} ({execution_mode} mode)\n"
