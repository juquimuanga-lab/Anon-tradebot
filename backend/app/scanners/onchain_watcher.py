...
# Pump.fun stream tasks keyed by the official Pump.fun program.
...
def _raw_transaction_instructions(
    ...
    """Return outer + inner instructions from getTransaction.

    Pump.fun create/create_v2 can be represented inside innerInstructions by
    some RPC/provider transaction shapes. Include those instructions so launch
    version verification can prove the instruction even when it is not exposed
    as an outer instruction.
    """
    ...
    inner_groups = (
        ...
        or meta.get("inner_instructions")
        or []
    )
...
def _extract_pumpfun_create_from_native_tx(
    ...
    # Some confirmed transaction representations expose the Pump.fun create
    # inside innerInstructions rather than the outer message instruction list.
    # Inspect those too so create/create_v2 verification does not unnecessarily
    # fall back to the CreateEvent.
    meta = getattr(
        tx,
        "meta",
        None,
    ...
    if meta is not None:
        inner_groups = (
            getattr(meta, "inner_instructions", None)
            or getattr(meta, "innerInstructions", None)
            or []
        )
    ...
    for group in inner_groups:
        instructions.extend(
            list(
                getattr(group, "instructions", None)
                or []
            )
        )

    account_keys = (
        getattr(message, "account_keys", None)
        or getattr(message, "accountKeys", None)
        or []
    )
...
def drain_smart_money_buys(rpc_url: str, wallets: list[str]) -> list[dict]:
    ...
    for wallet in wallets:
        ...
        if wallet_count:
            logger.info(
                "smart_money_queue_drained",
                extra={
                    "wallet": wallet,
                    "drained": wallet_count,
    ...
    while not stop_event.is_set():
        ...
        if not ws_url:
            ...
            logger.warning(
                ...
                f"reason=missing_primary_ws_url program_id={program_id}"
            )
        else:
            try:
                logger.info(
                ...
                async with websockets.connect(
                    ...
                    logger.info(
                        ...
                        f"transport=primary program_id={program_id} "
                    ...
                    while not stop_event.is_set():
                        ...
                        launch["watched_program"] = PUMPFUN_PROGRAM_ID
                ...
                logger.warning(
                    ...
                    f"program_id={program_id} fallback=alchemy_http"
                )
...
def _get_or_create_pumpfun_stream(
    ...
    """Create the Pump.fun stream keyed only by the official program."""
    program_id = PUMPFUN_PROGRAM_ID

    state = _pumpfun_streams.get(program_id)
    if state:
        task = state.get("task")
        if task and not task.done():
            return state
    ...
    task = asyncio.create_task(
        ...
        name=f"pumpfun-stream-{program_id[:8]}",
    )

    state["task"] = task
    _pumpfun_streams[program_id] = state
    return state
...
async def poll_new_pumpfun_mints(
    ...
    """Discover genuine Pump.fun launches from the official program only."""
    program_id = PUMPFUN_PROGRAM_ID
    ...
    for launch in _drain_pumpfun_queue(state["queue"]):
        ...
        watermark_key = f"pumpfun:program:{PUMPFUN_PROGRAM_ID}"
