...
from app.bot import handlers_admin, handlers_basic, handlers_wallet, rule_manager, handlers_doppler
...
async def set_bot_commands(application: Application) -> None:
    ...
    commands = [
        ...
        BotCommand("dopplerstatus", "Show Doppler/SPCX readiness"), BotCommand("enabledoppler", "Enable Doppler/SPCX sniper"), BotCommand("disabledoppler", "Disable Doppler/SPCX sniper"),
        BotCommand("setdopplersize", "Set Doppler buy size in SPCX"), BotCommand("approvespcx", "Approve SPCX for Doppler"),
        BotCommand("approveweth", "Approve Robinhood WETH for V4"), BotCommand("wethsnipe", "Snipe a Robinhood token with WETH"),
        BotCommand("setrule", "Create a Solana rule"), BotCommand("setrulepons", "Create a Robinhood/Pons ETH rule"), BotCommand("recoverent", "Recover token-account rent"),
        BotCommand("burnclose", "Burn and close token accounts"), BotCommand("guardian", "GO Guardian AI dashboard"), BotCommand("arbitrage", "Show arbitrage status"),
        BotCommand("enablearbitrage", "Enable arbitrage scanning"), BotCommand("disablearbitrage", "Disable arbitrage scanning"), BotCommand("arbscan", "Scan venue-constrained spreads"),
...
def build_application() -> Application:
    ...
    application.add_handler(CommandHandler("rules", rule_manager.rule_manager_start)); application.add_handler(CommandHandler("rulemanager", rule_manager.rule_manager_start)); application.add_handler(CommandHandler("listrules", handlers_basic.listrules)); application.add_handler(CommandHandler("activaterule", handlers_basic.activaterule)); application.add_handler(CommandHandler("balance", handlers_basic.balance)); application.add_handler(CommandHandler("positions", handlers_basic.positions_cmd)); application.add_handler(CommandHandler("history", handlers_basic.history)); application.add_handler(CommandHandler("guardian", handlers_admin.guardian_cmd))
    ...
    connect_conv = ConversationHandler(entry_points=[CommandHandler("connect", handlers_admin.connect_start)], states={handlers_admin.CONNECT_WAITING_KEY: [MessageHandler(filters.TEXT & ~filters.COMMAND | filters.Regex("^/cancel$"), handlers_admin.connect_receive)]}, fallbacks=[CommandHandler("cancel", handlers_admin.connect_receive)], name="connect_conversation"); application.add_handler(connect_conv)
    connectbscwallet_conv = ConversationHandler(entry_points=[CommandHandler("connectbscwallet", handlers_wallet.connectbscwallet_start)], states={handlers_wallet.BSC_CONNECT_WALLET_WAITING: [MessageHandler(filters.TEXT & ~filters.COMMAND | filters.Regex("^/cancel$"), handlers_wallet.connectbscwallet_receive)]}, fallbacks=[CommandHandler("cancel", handlers_wallet.connectbscwallet_receive)], name="connect_bsc_wallet_conversation"); application.add_handler(connectbscwallet_conv)
    connectrobinhoodwallet_conv = ConversationHandler(entry_points=[CommandHandler("connectrobinhoodwallet", handlers_wallet.connectrobinhoodwallet_start)], states={handlers_wallet.ROBINHOOD_CONNECT_WALLET_WAITING: [MessageHandler(filters.TEXT & ~filters.COMMAND | filters.Regex("^/cancel$"), handlers_wallet.connectrobinhoodwallet_receive)]}, fallbacks=[CommandHandler("cancel", handlers_wallet.connectrobinhoodwallet_receive)], name="connect_robinhood_wallet_conversation"); application.add_handler(connectrobinhoodwallet_conv)
    connectwallet_conv = ConversationHandler(entry_points=[CommandHandler("connectwallet", handlers_wallet.connectwallet_start)], states={handlers_wallet.CONNECT_WALLET_WAITING: [MessageHandler(filters.TEXT & ~filters.COMMAND | filters.Regex("^/cancel$"), handlers_wallet.connectwallet_receive)]}, fallbacks=[CommandHandler("cancel", handlers_wallet.connectwallet_receive)], name="connectwallet_conversation"); application.add_handler(connectwallet_conv)
    burnclose_conv = ConversationHandler(entry_points=[CommandHandler("burnclose", handlers_admin.burnclose_cmd)], states={handlers_admin.BURN_CLOSE_WAITING_ACCOUNTS: [MessageHandler(filters.TEXT & ~filters.COMMAND | filters.Regex("^/cancel$"), handlers_admin.burnclose_receive)]}, fallbacks=[CommandHandler("cancel", handlers_admin.burnclose_receive)], name="burnclose_conversation"); application.add_handler(burnclose_conv)
    rent_recovery_conv = ConversationHandler(entry_points=[CommandHandler("recoverent", handlers_admin.recoverent_cmd)], states={handlers_admin.RENT_RECOVERY_WAITING_SIGNATURES: [MessageHandler(filters.TEXT & ~filters.COMMAND | filters.Regex("^/(skip|cancel)$"), handlers_admin.recoverent_receive)]}, fallbacks=[CommandHandler("cancel", handlers_admin.recoverent_receive)], name="rent_recovery_conversation"); application.add_handler(rent_recovery_conv)
    setrule_pons_conv = ConversationHandler(entry_points=[CommandHandler("setrulepons", setrule_pons_start)], states={COLLECTING: [MessageHandler(filters.TEXT & ~filters.COMMAND | filters.Regex("^/(skip|cancel)$"), setrule_collect)]}, fallbacks=[CommandHandler("cancel", setrule_collect)], name="setrule_pons_conversation"); application.add_handler(setrule_pons_conv)
    setrule_fourmeme_conv = ConversationHandler(entry_points=[CommandHandler("setrulefourmeme", setrule_fourmeme_start)], states={COLLECTING: [MessageHandler(filters.TEXT & ~filters.COMMAND | filters.Regex("^/(skip|cancel)$"), setrule_collect)]}, fallbacks=[CommandHandler("cancel", setrule_collect)], name="setrule_fourmeme_conversation"); application.add_handler(setrule_fourmeme_conv)
    setrule_conv = ConversationHandler(entry_points=[CommandHandler("setrule", setrule_start)], states={COLLECTING: [MessageHandler(filters.TEXT & ~filters.COMMAND | filters.Regex("^/(skip|cancel)$"), setrule_collect)]}, fallbacks=[CommandHandler("cancel", setrule_collect)], name="setrule_conversation"); application.add_handler(setrule_conv)

    application.add_handler(MessageHandler(filters.Document.ALL, rule_manager.rule_manager_document)); application.add_handler(CommandHandler("cancel", rule_manager.rule_manager_cancel)); application.add_handler(CallbackQueryHandler(rule_manager.rule_manager_callback, pattern=r"^rulemgr:")); application.add_handler(CallbackQueryHandler(handlers_admin.action_confirmation_callback, pattern=r"^actionconfirm:")); application.add_handler(CallbackQueryHandler(handlers_admin.confirmation_callback, pattern=r"^confirm:")); application.add_handler(CallbackQueryHandler(handlers_admin.pumpfun_snipers_callback, pattern=r"^sniper:")); application.add_handler(CallbackQueryHandler(handlers_admin.guardian_callback, pattern=r"^guardian:")); application.add_handler(CallbackQueryHandler(handlers_admin.burnclose_callback, pattern=r"^burnclose:")); application.add_handler(CallbackQueryHandler(handlers_admin.rent_recovery_callback, pattern=r"^rent:")); application.add_handler(CallbackQueryHandler(handlers_wallet.disconnectrobinhoodwallet_callback, pattern=r"^rhdisconnect:"))
    return application
