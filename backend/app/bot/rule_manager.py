...
def _platform_label(platform: str) -> str:
    return {"solana": "SOLANA / Pump.fun", "pons": "ROBINHOOD / PONS (ETH)", "fourmeme": "FOUR.MEME / BSC"}.get(platform, platform.upper())


def _strategy_label(strategy: str) -> str:
    return {"fast": "⚡ Fast Sniper", "smart": "🧠 Smart Filter", "smart_money": "🐋 Smart Money Copy"}.get(strategy, strategy)
...
async def rule_manager_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "all":
        ...
        if not rules:
            await query.edit_message_text("📋 *Saved Rules*\n\nNo rules saved yet. Use Import JSON or /setrule.", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="rulemgr:home")]]))
            return
        lines = ["📋 *SAVED RULES*\n"]
        ...
        await query.edit_message_text("\n".join(lines), parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="rulemgr:home")]]))
        return
    if action == "lane" and len(parts) == 4:
        await _show_lane(update, parts[2], parts[3])
        return
    if action == "activate" and len(parts) == 3 and parts[2].isdigit():
        rule_id = int(parts[2])
        rules = await repo.get_rules_for_admin(update.effective_user.id)
        rule = next((r for r in rules if int(r.id) == rule_id), None)
        if not rule:
        ...
        if not activated:
            ...
            return
        if strategy == "smart_money" and not await repo.set_smart_money_rule(rule_id, update.effective_user.id):
            await query.edit_message_text("⚠️ Rule activated, but Smart Money assignment failed.")
            return
        await repo.write_audit_log(str(update.effective_user.id), "rule_manager_activate", {"rule_id": rule_id, "platform": rule.platform, "strategy": strategy})
...
async def _show_lane(update: Update, platform: str, strategy: str) -> None:
    ...
    rules = await repo.get_rules_for_admin(update.effective_user.id)
    lane_rules = [r for r in rules if getattr(r, "platform", "solana") == platform and getattr(r, "strategy", "smart") == strategy]
    ...
    buttons.append([InlineKeyboardButton("⬅️ Manager", callback_data="rulemgr:home"), InlineKeyboardButton("📥 Import", callback_data="rulemgr:import")])
...
async def rule_manager_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        for item in items:
            ...
            imported.append(await repo.create_rule(params, update.effective_user.id, activate=False))
        await repo.write_audit_log(str(update.effective_user.id), "rule_manager_import", {"count": len(imported), "rule_ids": [r.id for r in imported], "file": document.file_name or "upload.json"})
        ...
        await update.message.reply_text("\n".join(lines), parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚙️ Open Rule Manager", callback_data="rulemgr:home")], [InlineKeyboardButton("📋 View Saved Rules", callback_data="rulemgr:all")]]))
    except Exception as exc:
        logger.exception("rule_manager_import_failed")
        await update.message.reply_text("❌ *Rule import failed*\n\n" f"`{type(exc).__name__}: {exc}`\n\n" "No rule is activated automatically. Fix the JSON and upload it again.", parse_mode="Markdown")
