...
def evaluate_late_entry(token: TokenSnapshot, rule: RuleParams) -> tuple[bool, list[str], dict]:
    ...
    short_runup = pct(current_price, previous_price) if previous_price > 0 and sample_age is not None and 0 <= sample_age <= 3.0 else None
    breakdown = {
        "age_seconds": round(age, 2), "market_cap_usd": round(current_mc, 2), "price_usd": current_price,
        "first_price_usd": first_price, "first_market_cap_usd": first_mc,
    ...
    reasons: list[str] = []
    if current_mc >= rule.late_entry_soft_market_cap_usd and age > rule.late_entry_max_age_seconds:
        reasons.append(f"late entry: market cap ${current_mc:,.0f} at {age:.1f}s exceeds the {rule.late_entry_max_age_seconds:.1f}s entry window")
    if current_mc >= rule.late_entry_hard_market_cap_usd and distance_from_peak is not None and distance_from_peak < rule.late_entry_required_pullback_pct:
        reasons.append(f"late entry: ${current_mc:,.0f} market cap is only {distance_from_peak:.1f}% below the local high; requires {rule.late_entry_required_pullback_pct:.1f}% pullback")
    if current_mc >= rule.late_entry_soft_market_cap_usd and runup_from_first is not None and runup_from_first >= rule.late_entry_max_runup_from_first_pct and distance_from_peak is not None and distance_from_peak <= rule.late_entry_near_high_pct:
        reasons.append(f"late entry: price is +{runup_from_first:.0f}% from first observed price and within {distance_from_peak:.1f}% of the local high")
