...
class TokenSnapshot:
    ...
    # None means "unknown / could not fetch"
...
def evaluate_hard_filters(
    ...
    """
    Reject fast on any hard filter violation.
    ...
    if token.creator_wallet and token.creator_wallet in rule.creator_denylist:
        reasons.append(
            f"creator {token.creator_wallet} is denylisted"
        )

    if (
        rule.creator_allowlist
        and token.creator_wallet not in rule.creator_allowlist
    ):
        reasons.append("creator not in allowlist")

    if token.liquidity_usd < rule.min_liquidity_usd:
        reasons.append(
            f"liquidity ${token.liquidity_usd:,.0f} below min ${rule.min_liquidity_usd:,.0f}"
        )

    # Unknown holders are reported explicitly instead of being treated as zero.
    ...
    if token.age_seconds > rule.max_age_seconds:
        reasons.append(
            f"age {token.age_seconds:.0f}s exceeds max {rule.max_age_seconds}s"
        )

    if (
        rule.bonding_curve_phase != "any"
        and token.bonding_curve_phase != rule.bonding_curve_phase
    ):
        reasons.append(
            f"bonding curve phase {token.bonding_curve_phase} != required {rule.bonding_curve_phase}"
        )

    if (
        rule.min_market_cap_usd is not None
        and token.market_cap_usd < rule.min_market_cap_usd
    ):
        reasons.append(
            f"market cap ${token.market_cap_usd:,.0f} below min ${rule.min_market_cap_usd:,.0f}"
        )

    if (
        rule.max_market_cap_usd is not None
        and token.market_cap_usd > rule.max_market_cap_usd
    ):
        reasons.append(
            f"market cap ${token.market_cap_usd:,.0f} above max ${rule.max_market_cap_usd:,.0f}"
        )
